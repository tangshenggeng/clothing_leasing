		...
		<input type="hidden" id="classifyId_edit" name="classifyId" >
			...
			<div class="layui-form-item">
			 ...
			 <div class="layui-form-item">
			    ...
			    <div class="layui-input-block">
			      <input type="radio" name="isShow" value="展示" title="展示" >
			      <input type="radio" name="isShow" value="隐藏" title="隐藏" checked="checked">
			    </div>
			  </div>
			<div class="layui-form-item">
				<div class="layui-input-block">
...
function renderTb() {
	...
	layui.use(['table','util','form'], function() {
		...
		table.render({
			elem : '#dataListTb',
			height :400,
			url : '${PATH}/classify/getHideClassifyList',
			text : {
				none : '未找到数据'
			},
			size : 'sm ',
	...
	$("#exceptionCustBtn").click(function() {
		layui.use([ 'table', 'layer' ], function() {
			...
			layer.confirm('确认隐藏分类吗？', function(index) {
				...
				$.ajax({
					url : "${PATH}/classify/showClassifyByIds",
					method : "POST",
					contentType : "application/json",//必须指定，否则会报415错误
					dataType : 'json',
					data : JSON.stringify(ids),
