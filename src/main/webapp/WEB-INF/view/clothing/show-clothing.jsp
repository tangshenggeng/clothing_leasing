...
<div style="display: none;" id="editModal">
		...
		<input type="hidden" id="clothingId_edit" name="clothingId" >
			...
			<div class="layui-form-item">
			    ...
			    <div class="layui-input-block">
			      <input type="radio" name="clothingState" value="展示" title="展示" checked="checked">
...
function renderTb() {
	...
	layui.use(['table','util','form'], function() {
		...
		table.render({
			elem : '#dataListTb',
			height :400,
			url : '${PATH}/clothing/getShowClothingList',
			text : {
				none : '未找到数据'
			},
			size : 'sm ',
	...
	$("#exceptionCustBtn").click(function() {
		layui.use([ 'table', 'layer' ], function() {
			var table = layui.table, layer = layui.layer;
			layer.confirm('确认隐藏服装吗？', function(index) {
				var checkStatus = table.checkStatus('dataListTbId');
				var datas = checkStatus.data
				var ids = new Array();
				$(datas).each(function() {
				...
				$.ajax({
					url : "${PATH}/clothing/hideClothingByIds",
					method : "POST",
					contentType : "application/json",//必须指定，否则会报415错误
					dataType : 'json',
					data : JSON.stringify(ids),
